const card_container =  document.getElementById("card-container-revolution");
...
function display_cards() {
  ...
  let filtered_card = dati_card.filter(card => saved.includes(card.id))
  filtered_card.forEach(rv => {
    card_container.innerHTML +=   
    `
  ...
  if (filtered_card.length == 0) {
    ...
    <div class="no-card">
      ...
      <p>Aggiungine uno cliccando la bandierina</p>
    </div>
    `
  }
